function TweenEvent(){}
TweenEvent.prototype = new Event("TweenEvent");
...
function Tween(_data)
{
	...
	function onStart()
	{
		BroadcastCenter.sendEvent($(self.data).children("onStart").prop("outerHTML"));
	}

	function onUpdate()
	{
		BroadcastCenter.sendEvent($(self.data).children("onUpdate").prop("outerHTML"));
	}

	function onComplete()
	{
		BroadcastCenter.sendEvent($(self.data).children("onComplete").prop("outerHTML"));
	}
}
...
Tween.Delay = function(_data)
{
	...
	function myFunc()
	{
		BroadcastCenter.sendEvent(_data.outerHTML);
	}
}
Tween.Delay.list = {};
Tween.DelayEvent = function(){}
Tween.DelayEvent.prototype = new Event("DelayEvent");
...
Tween.init = function()
{
	...
	BroadcastCenter.addEventListener(TweenEvent.prototype.type,function(event,data){
		var tween = new Tween(data);
	});
	BroadcastCenter.addEventListener(Tween.DelayEvent.prototype.type,function(event,data){
		var delay = new Tween.Delay(data);
	});
}

FactoryMap["TWEEN"] = TweenEvent;
FactoryMap["DELAY"] = Tween.DelayEvent;
Tween.init();